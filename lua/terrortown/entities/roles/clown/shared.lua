...
if SERVER then
	...
	hook.Add("TTTBeginRound", "GrabAllClowns", function()
		local players = player.GetAll()	
		local clownTable = {}
		for i = 1, #players do
			local ply = players[i]
			if ply:GetSubRole() == ROLE_CLOWN then
				clownTable[#clownTable + 1] = ply
			end
		end
		hook.Add("TTT2JesterModifyList", "AddClownToJesterList", function(table)
			local jesterTable = table
			for i = 1, #clownTable do
				local clown = clownTable[i]
				jesterTable[#jesterTable + 1] = clown
			end
		end)
	end)
	...
	hook.Add("TTT2SpecialRoleSyncing", "TTT2RoleClown", function(ply, tbl)
		...
		for clown in pairs(tbl) do
			...
			if ply:GetSubRole() ~= ROLE_CLOWN and clown:GetSubRole() == ROLE_CLOWN then
				...
				if ply:GetTeam() ~= TEAM_CLOWN then
					tbl[clown] = {ROLE_JESTER, TEAM_JESTER}
				else
					tbl[clown] = {ROLE_CLOWN, TEAM_CLOWN}
				end
			end
		end
	end)
